...
func _build_entity_card(code: String, label: String, accent: Color, type: EntityType) -> Button:
	...
	s.set_border_color(accent); s.set_border_width_all(0); s.border_width_top = 2
	card.add_theme_stylebox_override("normal", s)

	var sh := _flat_style(C_BG_LIGHT.lightened(0.08), 6)
	sh.set_border_color(accent); sh.set_border_width_all(0); sh.border_width_top = 2
	card.add_theme_stylebox_override("hover", sh)

	var sp := _flat_style(accent.darkened(0.6), 6)
	sp.set_border_color(accent); sp.set_border_width_all(0); sp.border_width_top = 2
...
func _refresh_attribute_panel() -> void:
	...
	match selected_entity:
		EntityType.TRANSCEIVER:
			...
			_add_slider("Tx Power",  0.0,  10.0,   _prop_float("power",     5.0),    "dBm", C_BLUE,  func(v): _write("power",     int(v)), true)
			_add_slider("Frequency", 30.0, 3000.0, _prop_float("frequency", 1000.0), "MHz", C_BLUE,  func(v): _write("frequency", v),      true)
			_add_slider("Height",    0.0,  10.0,   _node_int("height",      5),      "m",   C_BLUE,  func(v): _write_node("height", int(v)), true)

		EntityType.JAMMER:
			_attr_header.text = "Jammer"
			_attr_header.add_theme_color_override("font_color", C_AMBER)
			_add_accent_bar(C_AMBER)
			_add_slider("Power",     0.0,  10.0,   _prop_float("power",     5.0),    "dBm", C_AMBER, func(v): _write("power",     int(v)), true)
			_add_slider("Frequency", 30.0, 3000.0, _prop_float("frequency", 1000.0), "MHz", C_AMBER, func(v): _write("frequency", v),      true)
			_add_dropdown("Bandwidth", ["Narrow","Medium","Wide"], _prop_int("jammer_bandwidth", 1), C_AMBER, func(v): _write("jammer_bandwidth", v))
			_add_slider("Height",    0.0,  10.0,   _node_int("height",      5),      "m",   C_AMBER, func(v): _write_node("height", int(v)), true)

		EntityType.SENSOR:
			_attr_header.text = "Sensor"
			_attr_header.add_theme_color_override("font_color", C_RED)
			_add_accent_bar(C_RED)
			_add_slider("Sensitivity", 0.0, 10.0, _prop_float("sensitivity", 3.0), "dBm", C_RED, func(v): _write("sensitivity", int(v)), true)
			_add_dropdown("Bandwidth", ["Narrow","Medium","Wide"], _prop_int("sensor_bandwidth", 1), C_RED, func(v): _write("sensor_bandwidth", v))
			_add_slider("Height",   0.0,  10.0,  _node_int("height",     5),      "m",   C_RED,  func(v): _write_node("height", int(v)), true)
			_add_toggle("Scanning", _prop_bool("is_scanning", true), C_RED, func(v): _write("is_scanning", v))
...
func _add_slider(label: String, min_v: float, max_v: float, current: float,
		unit: String, accent: Color, on_change: Callable, integers: bool = false) -> void:
	...
	slider.value_changed.connect(func(v): spin.set_value_no_signal(v); on_change.call(v))
	spin.value_changed.connect(func(v):  slider.set_value_no_signal(v); on_change.call(v))


func _add_dropdown(label: String, options: Array, current_idx: int, accent: Color, on_change: Callable) -> void:
	var vbox := _make_row_container()
	var hbox := HBoxContainer.new()
	...
	for opt in options: dd.add_item(opt)
...
func _component() -> Node:
	if not selected_node: return null
	match selected_entity:
		EntityType.TRANSCEIVER: return selected_node.find_child("Transceiver")
		EntityType.JAMMER:      return selected_node.find_child("Jammer")
		EntityType.SENSOR:      return selected_node.find_child("Sensor")
	return null
...
func _write_node(p: String, value) -> void:
	if selected_node and p in selected_node: selected_node.set(p, value)
...
func _apply_style(control: Control, bg: Color, border: Color,
		top: int, bottom: int, left: int, right: int) -> void:
	var s := StyleBoxFlat.new()
	s.bg_color = bg
...
func _make_label(text: String, color: Color, size: int, expand: bool = false) -> Label:
	...
	if expand: lbl.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	return lbl
