...
func _get_unit_spatial_data(unit: Node, terrain: Node) -> Dictionary:
	var px: Vector2 = unit.get("global_position") if unit.get("global_position") != null else Vector2.ZERO
...
func _rebuild_sources() -> void:
	...
	if height_grid == null: height_grid = []
	...
	for tx in get_tree().get_nodes_in_group("transceivers"):
		...
		if terrain != null and height_grid.size() > 0:
			terrain_loss = PhysicsEngine.compute_terrain_loss(tx_px, srx_px, z_tx, z_rx, height_grid, map_origin, map_scale)
			
		var rx_power = PhysicsEngine.SENSOR_BALANCE_RATIO * PhysicsEngine.calculate_received_power(power, z_tx, z_rx, freq, dist, terrain_loss)
	...
	for jammer in get_tree().get_nodes_in_group("jammers"):
		...
		var terrain_loss := 1.0
		if terrain != null and height_grid.size() > 0:
			terrain_loss = PhysicsEngine.compute_terrain_loss(j_px, srx_px, z_j, z_rx, height_grid, map_origin, map_scale)
			
		var rx_power = PhysicsEngine.calculate_received_power(power, z_j, z_rx, freq, dist, terrain_loss)
		var jammer_power_at_rx = PhysicsEngine.JAMMER_BALANCE_RATIO * rx_power * PhysicsEngine.BANDWIDTH_POWER[bw_idx]
...
func _draw_grid(pr: Rect2) -> void:
	...
	while f <= FREQ_MAX:
		...
		draw_string(_font, Vector2(5.0, y + 4.0), "%.0f" % f, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, C_LABEL)
		f += step
...
func _draw_header() -> void:
	...
	draw_string(_font, Vector2(btn_rect.position.x + 10.0, btn_rect.position.y + 18.0), text, HORIZONTAL_ALIGNMENT_LEFT, -1, 12, text_col)
...
func _nice_step(rough: float) -> float:
	...
	var ratio := rough / mag
	if   ratio < 2.0: return mag
	elif ratio < 5.0: return 2.0 * mag
	else:             return 5.0 * mag
